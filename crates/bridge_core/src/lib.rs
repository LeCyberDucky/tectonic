...
pub trait DriverHooks {
    ...
    /// Returning the output generated by a "shell escape" evaluation
...
impl<'a> CoreBridgeState<'a> {
    ...
    fn pipe_open(&mut self, command: &str) -> *mut InputHandle {
        if self.security.allow_shell_escape() {
            let input_handle = match self.hooks.sysrq_piped_input(command, self.status) {
                Ok(open_result) => match open_result {
                    OpenResult::Ok(input) => input,
                    OpenResult::NotAvailable => return ptr::null_mut(),
                    OpenResult::Err(e) => {
                        tt_warning!(self.status, "open of output generated by command {} failed", command; e);
                        return ptr::null_mut();
                    }
                },
                Err(e) => {
            ...
            self.input_handles.push(Box::new(input_handle));
            &mut **self.input_handles.last_mut().unwrap()
        } else {
            tt_error!(
                self.status,
...
/// Run a shell command and collect the generated output 
/// ///
/// # Safety
///
/// This function is unsafe because it dereferences raw pointers from C and accepts a raw C string.
#[no_mangle]
pub unsafe extern "C" fn ttbc_input_pipe_open(
    es: &mut CoreBridgeState,
    cmd: *const u16,
    len: libc::size_t,
) -> *mut InputHandle {
