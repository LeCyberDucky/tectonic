...
impl DriverHooks for BridgeState {
    ...
    fn sysrq_piped_input(
        &mut self,
        command: &str,
        status: &mut dyn StatusBackend,
    ) -> StdResult<OpenResult<InputHandle>, SystemRequestError> {
        #[cfg(unix)]
        const SHELL: &[&str] = &["sh", "-c"];
        ...
        if let Some(work) = self.shell_escape_work.as_ref() {
            ...
            if command.starts_with("|") {
                println!("Yup, that's the right command!");
            }

            let (piped_input, command) = if let Some(stripped_command) = command.strip_prefix("|") {
                (true, stripped_command.replacen("'", "", 2))
            } else {
                (false, command.into())
            };

            let command_execution = Command::new(SHELL[0])
            .args(&SHELL[1..])
            .arg(&command)
            .current_dir(work.root()).output();

            match command_execution
            {
                Ok(output) => match output.status.code() {
                    Some(0) => {
                        // Write output generated by command to file
                        let output = output.stdout;
                        // TODO: Figure out a better file name. The command itself could easily be an invalid path name 
                        let output_path = work.root().join(command);
                        println!("Command output will be stored in: {}", output_path.display());
                        let mut f = File::create(&output_path).map_err(|e| {
                            tt_error!(status, "failed to create file `{}`", output_path.display(); e.into());
                            SystemRequestError::Failed
                        })?;
                        f.write_all(&output).map_err(|e| {
                            tt_error!(status, "failed to write file `{}`", output_path.display(); e.into());
                            SystemRequestError::Failed
                        })?;
                        let output_path = output_path.to_str().ok_or_else(|| SystemRequestError::Failed)?;
                        Ok(self.input_open_name(output_path, status))
                    },
                    Some(n) => {
                        tt_warning!(status, "command exited with error code {}", n);
                        Err(SystemRequestError::Failed)
